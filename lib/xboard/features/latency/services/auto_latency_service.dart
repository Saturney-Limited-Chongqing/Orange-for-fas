...
class AutoLatencyService {
  ...
  void _cleanExpiredCache() {
    ...
    _proxyTestCache.removeWhere((proxyName, lastTestTime) {
      final isExpired =
          now.difference(lastTestTime).inMinutes >= _cacheMinutes * 2;
      if (isExpired) {
    ...
    if (expiredEntries.isNotEmpty) {
      _logger.debug(
          'AutoLatencyService', '清理过期缓存: ${expiredEntries.join(', ')}');
    }
  }
      ...
      if (!forceTest) {
        ...
        if (_lastTestedProxy == currentProxy.name && _lastTestTime != null) {
          ...
          if (timeSinceLastTest.inSeconds < 5) {
            _logger.debug(
                '代理 ${currentProxy.name} 刚刚测试过(${timeSinceLastTest.inSeconds}s前)，跳过重复测试');
            return;
          }
        }
      }
  ...
  Future<void> testCurrentGroupNodes({int maxNodes = 5}) async {
    ...
    try {
      ...
      final nodesToTest =
          currentGroup.all.where(_isDisplayableProxy).take(maxNodes).toList();
      if (nodesToTest.isEmpty) {
        _logger.debug('当前组没有可测速节点，跳过批量测试');
        return;
      }
  ...
  Proxy? _getCurrentSelectedProxy() {
    ...
    try {
      ...
      final fallbackProxy = currentGroup.all.firstWhere(
        _isDisplayableProxy,
        orElse: () => currentGroup.all.first,
      );
      if (!_isDisplayableProxy(fallbackProxy)) {
        _logger.debug('备用代理不是可显示节点，跳过当前节点测试: ${fallbackProxy.name}');
        return null;
      }
    ...
    if (mode == Mode.global) {
      ...
      for (final group in groups) {
        ...
        if (selectedProxyName != null && selectedProxyName.isNotEmpty) {
          final referencedGroup =
              groups.where((g) => g.name == selectedProxyName).firstOrNull;
  ...
  Proxy? _getSelectedProxyFromGroup(
    ...
    if (group.type == GroupType.URLTest || group.type == GroupType.Fallback) {
      ...
      if (selectedProxyName.isNotEmpty) {
        final referencedGroup =
            allGroups.where((g) => g.name == selectedProxyName).firstOrNull;
        if (referencedGroup != null) {
          return _getSelectedProxyFromGroup(
              referencedGroup, selectedMap, allGroups);
        } else {
    ...
    if (realNodeName.isNotEmpty &&
        group.all.any((p) => p.name == realNodeName)) {
      final proxy = group.all.firstWhere((proxy) => proxy.name == realNodeName);
      final referencedGroup = allGroups
          .where((candidate) => candidate.name == realNodeName)
          .firstOrNull;
      if (referencedGroup != null) {
      ...
      if (!_isDisplayableProxy(proxy)) {
        _logger.debug('节点不是可显示代理，跳过: ${proxy.name}');
        return null;
      }
  ...
  String _firstDisplayableProxyName(Group group) {
    return group.all
        .firstWhere(
          _isDisplayableProxy,
          orElse: () => Proxy(name: '', type: ''),
        )
        .name;
  }
